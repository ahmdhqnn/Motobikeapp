...
class BikeSource {
  static Future<List<Bike>?> fetchFeaturedBikes() async {
    try {
      // FirebaseFirestore.instance.collection('Bikes').where('rating',isGreaterThan: 4.5).limit(3);
  ...
  static Future<List<Bike>?> fetchNewsBikes() async {
    try {
      final ref = FirebaseFirestore.instance.collection('Bikes').orderBy('release', descending: true).limit(4);
      final queryDocs = await ref.get();
      return queryDocs.docs.map((doc) => Bike.fromJson(doc.data())).toList();
    } catch (e) {
      log(e.toString());
